  ...
  ({
    ...
    async function registerIdentifier(identifier: string) {
      try {
        ...
        const tx = await contract.registerIdentifier(
          String(`${identifier}.eth`),
        );
        const response = await tx.wait;
          ...
          <div className="flex items-center justify-between space-x-4">
            ...
            {activeAccount?.address && (
              <Button
                ...
                className="block bg-black text-white dark:bg-white dark:text-black"
                type="button"
              >
                AI-Wallet
              </Button>
